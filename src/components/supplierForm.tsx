...
import {
  ...
  getSupplierById,
  updateSupplier,
} from "~/data/db/schemas/suppliers";
...
export default function SupplierForm(props: DialogProps) {
  let nameRef!: HTMLInputElement;
  let phoneNumberRef!: HTMLInputElement;
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      if (props.id() == 0) {
        ...
        await props.refetch();
        props.setOpen(false);
        return toast.success(`Proveedor ${res.name} cadastrado!`);
      }
      ...
      await props.refetch();
      props.setOpen(false);
      return toast.success(`Proveedor ${res.name} editado!`);
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Ocorrio un error inesperado.";
      toast.error(message);
    }
  };
  ...
  createEffect(() => {
    ...
    phoneNumberRef.value = item.phoneNumber?.toString() ?? "";
          ...
          <form class="px-6 py-6 flex flex-col gap-5" onSubmit={handleSubmit}>
            ...
            <Show when={(props.id?.() ?? 0) > 0}>
              <p class="text-sm text-gray-400 text-center">
                Editando fornecedor ID: {props.id?.()}
              </p>
            </Show>
