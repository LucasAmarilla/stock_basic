...
import { deleteProduct, getProductsList } from "~/data/lib/products";
...
export default function Products() {
  ...
  const [result, { refetch }] = createResource(
    ...
    ([currentPage, currentSearch]) =>
      getProductsList(currentPage, 10, currentSearch),
  );
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      await revalidate(getProductsList.keyFor(page(), 10, submittedSearch()));
      await refetch();
                        ...
                        {(produto) => (
                          <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 font-medium text-gray-800">
                              {produto.products_table.name}
                            </td>

                            <td class="px-6 py-4 text-emerald-600 font-semibold">
                              G$ {produto.products_table.price}
                            </td>

                            <td class="px-6 py-4 text-gray-600">
                              G$ {produto.products_table.paidPrice}
                            </td>

                            <td class="px-6 py-4 text-gray-700">
                              {produto.products_table.quantity ?? 0}
                            </td>

                            <td class="px-6 py-4 text-gray-700">
                              {produto.supplier_table?.name ?? "-"}
                            </td>
                            ...
                            <td class="px-6 py-4">
                              <div class="flex items-center justify-center gap-2">
                                <button
                                  ...
                                  title="Editar"
                                  onClick={() => {
                                    setEditId(produto.products_table.id);
                                    setOpen(true);
                                  }}
                                >
                                  ✏️
                                ...
                                <button
                                  ...
                                  title="Excluir"
                                  onclick={() =>
                                    handleDelete(produto.products_table.id)
                                  }
                                >
                                  🗑️
                                </button>
